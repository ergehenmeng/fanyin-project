...
(function ($, undefined) {
    ...
    var rules = {
		...
		staffId : {
			validator : function(value) {
				return /^([a-zA-Z]{1,2}[0-9]{4})?$/.test(value);
			},
			message : '员工工号输入有误，标准格式如[RD0001]'
		},
        ...
        password: {
            validator: function (value) { return $.string.isSafePassword(value)&&value.length<17; },
            message: "输入的内容必须是6-16字母数与字组合格式."
        },
        regexPassword: {
            validator: function (value) { return /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{8,16}$/.test(value); },
            message: "输入的内容必须是8-16字母数与字组合格式."
        },        
        //  输入的字符必须是指定的内容相同
        equals: {
            validator: function (value, param) {
